 ...
 */
package org.opensearch.dataprepper.plugins.prometheus.service;
...
public class HttpPrometheusServiceTest {
    ...
    void setup() throws IOException {
        ...
        when(pluginMetrics.counter(PrometheusSinkService.HTTP_SINK_RECORDS_SUCCESS_COUNTER)).thenReturn(httpSinkRecordsSuccessCounter);
        when(pluginMetrics.counter(PrometheusSinkService.HTTP_SINK_RECORDS_FAILED_COUNTER)).thenReturn(httpSinkRecordsFailedCounter);
    ...
    void http_sink_service_test_output_with_multiple_records() throws NoSuchFieldException, IllegalAccessException {
        ...
        final PrometheusSinkService objectUnderTest = createObjectUnderTest(sinkRecords,prometheusSinkConfiguration);
    ...
    void http_sink_service_test_with_single_record_with_bearer_token() throws NoSuchFieldException, IllegalAccessException, JsonProcessingException {
        ...
        final String authentication = "          bearer_token:\n" +
                ...
                "            grant_type: client_credentials\n" +
                "            scope: httpSink" ;
        ReflectivelySetField.setField(PrometheusSinkConfiguration.class,prometheusSinkConfiguration,"authentication", objectMapper.readValue(authentication, AuthenticationOptions.class));
        final PrometheusSinkService objectUnderTest = createObjectUnderTest(1,prometheusSinkConfiguration);
        final Record<Event> eventRecord = new Record<>(JacksonEvent.fromMessage("{\"message\":\"c3f847eb-333a-49c3-a4cd-54715ad1b58a\"}"));
        objectUnderTest.output(List.of(eventRecord));
        verify(httpSinkRecordsSuccessCounter).increment(1);
